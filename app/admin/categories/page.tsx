        ...
        <div className="bg-white rounded-2xl shadow-sm border border-dusty-pink/10 overflow-hidden p-6">
          {loading ? (
             <div className="py-10 flex justify-center items-center">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-4">              {isAdding && (
              ...
              {categories.length === 0 && !isAdding ? (
                 <p className="text-sm text-charcoal/40 text-center py-6 col-span-full">No categories found. Add one above.</p>
              ) : (
                categories.map((cat) => (
                  <div key={cat.id} className="flex items-center justify-between p-3 rounded-lg bg-white border border-charcoal/10 hover:bg-cream/30 transition-colors group">
                    {editingId === cat.id ? (
                      <div className="flex items-center gap-3 flex-1">
                        <input
                          ...
                          className="flex-1 bg-white border border-charcoal/10 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:border-rose-gold"
                        />
                        <button onClick={() => handleUpdate(cat.id)} className="p-1.5 text-green-600 hover:bg-green-50 rounded-md transition-colors"><Check size={18} /></button>
                      ...
                      <>
                        <span className="text-charcoal font-medium text-sm">{cat.name}</span>
